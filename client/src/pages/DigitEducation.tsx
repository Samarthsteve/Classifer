...
import { Loader2, Pencil, Eraser, Trash2, Send, Monitor, CheckCircle2, Sparkles, Home, Brain, Zap, Eye } from "lucide-react";
import type { DrawingPayload, PredictionResult } from "@shared/schema";

interface DigitEducationProps {
    ...
    <div className="relative h-screen flex flex-col overflow-hidden">
        ...
        <div className="text-center max-w-lg mx-auto">
          <div className="relative mb-8">
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-36 h-36 rounded-full bg-purple-500/10 animate-pulse" />
            </div>
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-28 h-28 rounded-full bg-purple-500/20 animate-pulse" style={{ animationDelay: "150ms" }} />
            </div>
          ...
          >
            <span className="text-purple-400">Digit</span> Submitted
          </h2>
          ...
          {mainPrediction && (
            ...
            >
              ...
              <p 
                className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400"
                data-testid="text-tablet-prediction-class"
              >
                {mainPrediction.class}
              </p>
              <p 
                className="text-lg font-mono text-purple-400 mt-1"
          ...
          <p className="text-lg text-slate-400 leading-relaxed mb-8">
            Waiting for the display to be ready for the next digit
      ...
      <div className="absolute bottom-6 left-0 right-0 flex justify-center">
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 backdrop-blur-sm border border-slate-700/50">
          ...
          <span className="text-sm text-slate-400 font-medium">AI Digit Classification Complete</span>
        </div>
      </div>
      ...
      {/* Home button */}
      <Link href="/" data-testid="link-home-tablet-idle">
        <div className="absolute top-4 right-4 flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/60 backdrop-blur-sm border border-slate-700/50 text-slate-300 transition-all hover:bg-slate-700/60 hover:text-white cursor-pointer">
...
export default function DigitEducation({ 
  onSubmit, 
  isProcessing, 
  isConnected,
  isReconnecting,
  ...
  const getCanvasSize = useCallback(() => {
    ...
    const maxSize = window.innerWidth <= 1080 ? 350 : 600;
    ...
    <div className="relative h-screen flex flex-col overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950" ref={containerRef}>
      ...
      {(isProcessing || isReconnecting) && (
        <div className="absolute inset-0 z-50 bg-slate-950/95 flex flex-col items-center justify-center">
          ...
          <p className="mt-2 text-slate-500">
            {isReconnecting ? "Please wait" : "Processing your digit"}
          </p>
        </div>
      )}
      ...
      <div className="flex-none px-6 py-3">
        <div className="grid grid-cols-3 gap-3 max-w-md mx-auto">
          <div className="rounded-lg bg-gradient-to-br from-slate-800/40 to-slate-800/20 border border-slate-700/40 p-3 text-center">
            <Eye className="w-5 h-5 text-purple-400 mx-auto mb-1.5" />
            <p className="text-xs font-medium text-slate-300">Recognition</p>
            <p className="text-xs text-slate-500 mt-1">AI sees patterns</p>
          </div>
          <div className="rounded-lg bg-gradient-to-br from-slate-800/40 to-slate-800/20 border border-slate-700/40 p-3 text-center">
            <Brain className="w-5 h-5 text-purple-400 mx-auto mb-1.5" />
            <p className="text-xs font-medium text-slate-300">Analysis</p>
            <p className="text-xs text-slate-500 mt-1">Processes features</p>
          </div>
          <div className="rounded-lg bg-gradient-to-br from-slate-800/40 to-slate-800/20 border border-slate-700/40 p-3 text-center">
            <Zap className="w-5 h-5 text-purple-400 mx-auto mb-1.5" />
            <p className="text-xs font-medium text-slate-300">Prediction</p>
            <p className="text-xs text-slate-500 mt-1">Makes decision</p>
          </div>
        </div>
      </div>
      ...
      {/* Home button */}
      <Link href="/" data-testid="link-home-canvas">
